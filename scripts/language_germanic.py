    ...
    for index, directory in enumerate(directories):
      ...
      if len(target_languages) == 0 or directory in target_languages:
        ...
        for i, file in enumerate(files):
          ...
          data = process_file("../data_germanic/" + directory + "/" + file)
...
def process_text(line):
  ...
  for sentence in sentences:
    ...
    if len(sentence) > min_length:
      processed_sentences.extend([preprocess(" ".join(words[:3]))])
...
def handle_data():
  ...
  with open("../data/dataset_germanic.csv", 'w', newline='') as file:
    file.truncate(0)

  # Set column labels
  print("Inserting titles...")
  with open("../data/dataset_germanic.csv", 'w') as file:
  ...
  for language in languages:
    language.remove_by_length(min_length=30, max_length=max_length)
  ...
  for language in languages:
    if len(language.words) < max_rows:
      max_rows = len(language.words)

  print("Writing to csv")
  for language in languages:
    print(language)
    language.write_to_csv(max_rows, "../data/dataset_germanic.csv")
